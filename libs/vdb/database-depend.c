    ...
    if (rc == 0 && t.found) {
        if (vol) {
            rc = KDirectoryResolvePath
                (native, true, buf, blen, "%s/%s/%s", srv, vol, file);
        }
        else {
            rc = KDirectoryResolvePath
                (native, true, buf, blen, "%s/%s", srv, file);
        }
    }
...
static ErrType DependenciesType(rc_t rc) {
    if (rc == RC(rcDB, rcMgr, rcOpening, rcEncryptionKey, rcNotFound)) {
        return eNoEncInKfg;
    } else if (rc == RC(rcDB, rcMgr, rcOpening, rcEncryptionKey, rcNotOpen)) {
        return ePwdFileNotFound;
    } else if (rc == RC(rcDB, rcMgr, rcOpening, rcEncryptionKey, rcIncorrect)) {
        return eBadEncKey;
    }
    return eUnknown;
}
...
"run the 'sra-passwd.perl' script provided with the toolkit.\n"));
                break;
